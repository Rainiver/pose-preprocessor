...
def torch_pose_obj_data(batch_size=1):
    ...
    """
    body25_reg = pkl.load(open('regressors/body_25_openpose_joints.pkl', 'rb'), encoding="latin1").T
    face_reg = pkl.load(open('regressors/face_70_openpose_joints.pkl', 'rb'), encoding="latin1").T
    hand_reg = pkl.load(open('regressors/hands_42_openpose_joints.pkl', 'rb'), encoding="latin1").T
...
# for 2d to 3d convert
body_keypoints_weights = torch.tensor([
    0.6, 0.6,
    1.0, 10.0, 100.0,  # right arm
    1.0, 10.0, 100.0,  # left arm
    0.6, 0.6,
...
bodyhand_keypoints_weights = 0 * torch.ones((67, 1), dtype=torch.float)
bodyhand_keypoints_weights[:25, 0] = body_keypoints_weights[:, 0]


# for smpl fitting
body_keypoints_weights_fit = torch.tensor([
    0.6, 0.6,
    1.0, 10.0, 10.0,  # right arm
    1.0, 10.0, 10.0,  # left arm
    0.6, 0.6,
    1.0, 1.0,  # right knee and leg
    1.0, 1.0,  # left knee and leg
    0.8,  # left foot
...
bodyhand_keypoints_weights_fit = 0 * torch.ones((67, 1), dtype=torch.float)
bodyhand_keypoints_weights_fit[:25, 0] = body_keypoints_weights_fit[:, 0]
...
def batch_3djoints_loss(pc_bodyjoints, smpl_bodyjoints):
    ...
    if pc_bodyjoints.shape[1]==25:
        weights = body_keypoints_weights_fit
    elif pc_bodyjoints.shape[1] == 67:
        # weights = bodyhand_keypoints_weights
        weights = bodyhand_keypoints_weights_fit
    else:
        weights = joint_weights
    weights = weights.to(pc_bodyjoints.device)
    # print(weights)
    loss = mse_loss(pc_bodyjoints[:, :, :3], smpl_bodyjoints, reduction='none')
    # print(pc_bodyjoints.shape)
    # print(smpl_bodyjoints.shape)
